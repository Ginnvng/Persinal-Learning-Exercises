...
# [MOD] 主题切换：陈文静专属（paper_ink / quiet_jade）
THEME_MODE = "paper_ink"

THEME_PRESETS = {
    "paper_ink": {
        "cards": [
            {"bg": "#F7F7F4", "border": "#D6D8D0", "accent": "#7FA79A", "title": "#5A6B66", "text": "#333B39"},
            {"bg": "#F4F8F7", "border": "#C9DCD5", "accent": "#6BAF9D", "title": "#3F6C63", "text": "#2D4740"},
            {"bg": "#F6F5F9", "border": "#D8D3E5", "accent": "#8F87B5", "title": "#5E5878", "text": "#39364A"},
            {"bg": "#FAF7F2", "border": "#E3D9C8", "accent": "#B79A73", "title": "#80694A", "text": "#4D4030"},
            {"bg": "#F5F8FB", "border": "#CEDAE8", "accent": "#789EB8", "title": "#4B6579", "text": "#2F4454"},
        ],
        "final": {
            "bg": "#EEF3F1",
            "border": "#B7C9C2",
    ...
    "quiet_jade": {
        "cards": [
            {"bg": "#EDF6F2", "border": "#C5DED3", "accent": "#4F9C82", "title": "#2E6757", "text": "#23483D"},
            {"bg": "#F0F7F9", "border": "#C6DCE4", "accent": "#5C9CB4", "title": "#35667A", "text": "#274A58"},
            {"bg": "#F5F4FA", "border": "#D5D0E7", "accent": "#7B75AA", "title": "#524D76", "text": "#383557"},
            {"bg": "#F8F6F1", "border": "#DFD4C2", "accent": "#A88D63", "title": "#776243", "text": "#4E3F2C"},
            {"bg": "#F2F6F7", "border": "#CBD8D9", "accent": "#6F9797", "title": "#496B6B", "text": "#304747"},
        ],
        "final": {
            "bg": "#E9F2EF",
            "border": "#AFCBC2",
...
_selected_theme = THEME_PRESETS.get(THEME_MODE, THEME_PRESETS["paper_ink"])
...
BINDABLE_BLESSINGS = {
    "情感/情绪疏导": [
        "{name}，你的文雅和安静，不是退后，是一种很稳的力量",
        "{name}，像你的名字一样，把心安顿好，世界就会慢慢明亮",
    ...
    "成长/行动": [
        "{name}，文心能照路，静气能成事，你会越走越稳",
        "{name}，你身上的安静与笃定，会在关键时刻变成底气",
    ...
    "生活/趣味": [
        "{name}，把“文”写进热爱，把“静”放进节奏，日子会越过越顺",
...
class BlessingPopup:
    ...
    def _pick_random_position(self):
        ...
        margin = 16
        # [MOD] 陈文静专属：分布更舒展，避免过于“中心爆发”
        center_x = (screen_w - self.width) // 2
        center_y = (screen_h - self.height) // 2 + 20
        spread_x = max(170, int(screen_w * 0.33))
    ...
    def _draw_static_card(self):
        ...
        if self.is_final:
            inner_fill = mix_hex(self.theme["bg"], "#FFFFFF", 0.35)
            inner_border = mix_hex(self.theme["border"], "#FFFFFF", 0.15)
            draw_rounded_rect(
                self.canvas,
                card_x1 + 10,
                card_y1 + 10,
            ...
            line_color = mix_hex(self.theme["accent"], self.theme["title"], 0.30)
            # [MOD] 标题与装饰线分离，避免“Happy New Year !”贴着横线
            self.canvas.create_line(54, 30, self.width - 54, 30, fill=line_color, width=1)
            ...
            # [MOD] 陈文静专属：简净书笺风，去掉强烈赛博装饰，改为留白+细线
            ...
            self.canvas.create_line(
                ...
                card_x2 - 24,
                card_y1 + 36,
                fill=mix_hex(title_color, self.theme["border"], 0.35),
                width=1,
            )
            ...
            self.canvas.create_oval(
                ...
                card_x2 - 18,
                card_y1 + 40,
                fill=mix_hex(self.theme["bg"], "#FFFFFF", 0.62),
...
class FinalConfirmButton:
    ...
    def _draw_button(self, disabled):
        ...
        if disabled:
            bg_color = mix_hex(FINAL_THEME["bg"], "#C3CCC9", 0.45)
            border_color = mix_hex(FINAL_THEME["border"], "#C3CCC9", 0.45)
            text_color = mix_hex(FINAL_THEME["text"], "#B0BCB8", 0.45)
        else:
        ...
        self.canvas.create_line(
            ...
            self.width - 24,
            self.height - 16,
            fill=mix_hex(border_color, "#FFFFFF", 0.20),
            width=1,
        )
