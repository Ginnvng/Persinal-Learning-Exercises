...
# [MOD] 主题切换：王芳专属（fragrant_bloom / blush_breeze）
THEME_MODE = "fragrant_bloom"

THEME_PRESETS = {
    "fragrant_bloom": {
        "cards": [
            {"bg": "#FFF4F7", "border": "#F3C6D7", "accent": "#ECA3BE", "title": "#A95D79", "text": "#4E2D3C"},
            {"bg": "#FFF9F3", "border": "#F0DCC1", "accent": "#DDB58E", "title": "#8C6748", "text": "#4E3B2A"},
            {"bg": "#F4FCF8", "border": "#CAE8DA", "accent": "#81C9A8", "title": "#427A64", "text": "#2A4F42"},
            {"bg": "#F7F6FF", "border": "#DCD3F3", "accent": "#B4A4E4", "title": "#6D609D", "text": "#423B66"},
            {"bg": "#F5FAFF", "border": "#CFE1F3", "accent": "#8DB4E9", "title": "#4C6A9B", "text": "#31496A"},
        ],
        "final": {
            "bg": "#FFF3F7",
            "border": "#EFBFD1",
    ...
    "blush_breeze": {
        "cards": [
            {"bg": "#FFEFF5", "border": "#F4BDD0", "accent": "#E88FB1", "title": "#9A4F70", "text": "#4D2B3C"},
            {"bg": "#FFF6F1", "border": "#F2D8C8", "accent": "#D6A88B", "title": "#8A5F47", "text": "#4C392C"},
            {"bg": "#F3FBFF", "border": "#CAE0F1", "accent": "#84B4DB", "title": "#466D8E", "text": "#2E4961"},
            {"bg": "#F3FFF8", "border": "#CBE8DA", "accent": "#84C5A8", "title": "#457A64", "text": "#2C4F43"},
            {"bg": "#F8F4FF", "border": "#DED2F3", "accent": "#B3A0DF", "title": "#6C5C98", "text": "#423A63"},
        ],
        "final": {
            "bg": "#FDF2F7",
            "border": "#EAB8CB",
...
_selected_theme = THEME_PRESETS.get(THEME_MODE, THEME_PRESETS["fragrant_bloom"])
...
BINDABLE_BLESSINGS = {
    "情感/情绪疏导": [
        "{name}，像花香一样温柔地靠近生活，你会被世界温柔回应",
        "{name}，你的细腻和坚定都很珍贵，不必怀疑自己的光",
...
class BlessingPopup:
    ...
    def _pick_random_position(self):
        ...
        margin = 16
        # [MOD] 王芳专属：分布更舒展，整体更轻盈
        center_x = (screen_w - self.width) // 2
        center_y = (screen_h - self.height) // 2 + 14
        spread_x = max(170, int(screen_w * 0.32))
        spread_y = max(120, int(screen_h * 0.26))
    ...
    def _draw_static_card(self):
        ...
        if self.is_final:
            # [MOD] 王芳专属：花韵终章（丝带+花瓣章）
            inner_fill = mix_hex(self.theme["bg"], "#FFFFFF", 0.48)
            ...
            draw_rounded_rect(
                ...
                card_y1 + 38,
                10,
                fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.72),
                outline=mix_hex(self.theme["accent"], "#FFFFFF", 0.18),
                width=1,
            )

            # 左右花瓣章
            for cx, cy in ((78, 52), (self.width - 78, 52)):
                petal = mix_hex(self.theme["accent"], "#FFFFFF", 0.20)
                self.canvas.create_oval(cx - 7, cy - 2, cx + 7, cy + 2, fill=petal, outline="")
                self.canvas.create_oval(cx - 2, cy - 7, cx + 2, cy + 7, fill=petal, outline="")
            ...
            self.canvas.create_line(
                ...
                self.width - 66,
                self.height - 24,
                fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.30),
                width=1,
            )
        else:
            # [MOD] 王芳专属：花瓣便签（柔和圆章 + 角落花片）
            inner_fill = mix_hex(self.theme["bg"], "#FFFFFF", 0.56)
            inner_border_color = mix_hex(self.theme["border"], "#FFFFFF", 0.20)
            title_color = self.theme.get("title", self.theme["accent"])
            badge_text_color = mix_hex(title_color, "#1F2937", 0.50)
            ...
            # 左上角花片
            petal = mix_hex(self.theme["accent"], "#FFFFFF", 0.22)
            ...
            self.canvas.create_oval(
                ...
                card_x2 - 14,
                card_y2 - 12,
                fill=mix_hex(self.theme["bg"], "#FFFFFF", 0.70),
            ...
            self.canvas.create_line(
                ...
                card_x2 - 42,
                card_y2 - 18,
                fill=mix_hex(title_color, "#FFFFFF", 0.28),
                width=1,
            )
...
class FinalConfirmButton:
    ...
    def _draw_button(self, disabled):
        ...
        if disabled:
            bg_color = mix_hex(FINAL_THEME["bg"], "#B8B8C2", 0.46)
            border_color = mix_hex(FINAL_THEME["border"], "#B8B8C2", 0.46)
            text_color = mix_hex(FINAL_THEME["text"], "#A7A7B2", 0.44)
        else:
        ...
        # [MOD] 王芳专属按钮：花瓣点缀
        petal = mix_hex(border_color, "#FFFFFF", 0.22)
