...
# [MOD] 主题切换：户宇涵专属（petal_mist / warm_jade）
THEME_MODE = "petal_mist"

THEME_PRESETS = {
    "petal_mist": {
        "cards": [
            {"bg": "#FFF4F7", "border": "#F0C9D8", "accent": "#EFA1BE", "title": "#A75A79", "text": "#4B2A38"},
            {"bg": "#F5F9FF", "border": "#CBDCF7", "accent": "#8EB0E9", "title": "#4B6798", "text": "#2E4368"},
            {"bg": "#F3FBF8", "border": "#CBE7DC", "accent": "#7CC6AA", "title": "#3F7A67", "text": "#294E42"},
            {"bg": "#FFF8F0", "border": "#EEDBC2", "accent": "#DAB48D", "title": "#8C6842", "text": "#4D3A25"},
            {"bg": "#F7F4FF", "border": "#DDD1F4", "accent": "#AFA0E1", "title": "#675B9A", "text": "#3F3764"},
        ],
        "final": {
            "bg": "#FDF5F8",
            "border": "#E9BFD0",
    ...
    "warm_jade": {
        "cards": [
            {"bg": "#F2FFF8", "border": "#C8EAD9", "accent": "#7EC8A9", "title": "#3F7C63", "text": "#2A4F41"},
            {"bg": "#F6FBFF", "border": "#CCE1F0", "accent": "#8EB9D8", "title": "#4B6F88", "text": "#2F4858"},
            {"bg": "#FFF8F3", "border": "#EFDCCB", "accent": "#D7B295", "title": "#8D6A4F", "text": "#523D2E"},
            {"bg": "#F9F7FF", "border": "#DDD5F2", "accent": "#AFA5DA", "title": "#676096", "text": "#403B62"},
            {"bg": "#F7FFFD", "border": "#CEE8E0", "accent": "#84CABB", "title": "#41786E", "text": "#2A514A"},
        ],
        "final": {
            "bg": "#F1FFF9",
            "border": "#BFE2D2",
...
_selected_theme = THEME_PRESETS.get(THEME_MODE, THEME_PRESETS["petal_mist"])
...
BINDABLE_BLESSINGS = {
    "情感/情绪疏导": [
        "{name}，你的心像一扇温暖的门，能接住风雨，也能迎进阳光",
        "{name}，世界很大，但你心里的安定感会一直给你方向",
...
class BlessingPopup:
    ...
    def _pick_random_position(self):
        ...
        margin = 16
        # [MOD] 新风格：轻柔扩散分布，重心略偏上
        center_x = (screen_w - self.width) // 2
        center_y = (screen_h - self.height) // 2 - 6
        spread_x = max(180, int(screen_w * 0.34))
        spread_y = max(105, int(screen_h * 0.20))
    ...
    def _draw_static_card(self):
        ...
        if self.is_final:
            # [MOD] 新风格：柔雾花笺终章（更温柔，不走科幻）
            inner_fill = mix_hex(self.theme["bg"], "#FFFFFF", 0.42)
            inner_border = mix_hex(self.theme["border"], "#FFFFFF", 0.20)
            draw_rounded_rect(
                self.canvas,
                card_x1 + 11,
                card_y1 + 11,
            ...
            self.canvas.create_line(56, 34, self.width - 56, 34, fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.20), width=1)
            self.canvas.create_line(68, 72, self.width - 68, 72, fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.35), width=1)

            # 花瓣点缀
            for cx, cy in ((84, 52), (self.width - 84, 52)):
                self.canvas.create_oval(cx - 5, cy - 2, cx + 5, cy + 2, fill=self.theme["accent"], outline="")
                self.canvas.create_oval(cx - 2, cy - 5, cx + 2, cy + 5, fill=self.theme["accent"], outline="")
                self.canvas.create_oval(cx - 2, cy - 2, cx + 2, cy + 2, fill=self.theme["title"], outline="")
            ...
            self.canvas.create_line(
                ...
                self.width - 62,
                self.height - 24,
                fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.34),
                width=1,
            )
        else:
            # [MOD] 新风格：柔和小笺（清爽、细腻）
            inner_fill = mix_hex(self.theme["bg"], "#FFFFFF", 0.52)
            inner_border_color = mix_hex(self.theme["border"], "#FFFFFF", 0.20)
            title_color = self.theme.get("title", self.theme["accent"])
            badge_text_color = mix_hex(title_color, "#1F2937", 0.58)
            ...
            draw_rounded_rect(
                ...
                card_y1 + 34,
                10,
                fill=mix_hex(self.theme["accent"], "#FFFFFF", 0.78),
                outline=mix_hex(self.theme["accent"], "#FFFFFF", 0.22),
                width=1,
            )
            ...
            self.canvas.create_oval(
                ...
                card_x2 - 18,
                card_y1 + 38,
                fill=mix_hex(self.theme["bg"], "#FFFFFF", 0.68),
            ...
            # 底部水纹
            ripple = mix_hex(self.theme["accent"], "#FFFFFF", 0.42)
            self.canvas.create_line(card_x1 + 42, card_y2 - 18, card_x2 - 42, card_y2 - 18, fill=ripple, width=1)
            self.canvas.create_line(card_x1 + 54, card_y2 - 14, card_x2 - 54, card_y2 - 14, fill=ripple, width=1)
            ...
            self.canvas.create_line(
                ...
                card_x2 - 44,
                card_y1 + 40,
                fill=mix_hex(title_color, "#FFFFFF", 0.22),
                width=1,
            )
...
class FinalConfirmButton:
    ...
    def _draw_button(self, disabled):
        ...
        if disabled:
            bg_color = mix_hex(FINAL_THEME["bg"], "#A8B7B2", 0.44)
            border_color = mix_hex(FINAL_THEME["border"], "#A8B7B2", 0.44)
            text_color = mix_hex(FINAL_THEME["text"], "#AAB5B1", 0.45)
        else:
        ...
        self.canvas.create_line(
            ...
            self.width - 28,
            self.height - 16,
            fill=mix_hex(border_color, "#FFFFFF", 0.24),
            width=1,
        )
        petal = mix_hex(border_color, "#FFFFFF", 0.20)
        self.canvas.create_oval(20, 28, 30, 34, fill=petal, outline="")
        self.canvas.create_oval(23, 25, 27, 37, fill=petal, outline="")
        self.canvas.create_oval(self.width - 31, 28, self.width - 21, 34, fill=petal, outline="")
        self.canvas.create_oval(self.width - 28, 25, self.width - 24, 37, fill=petal, outline="")
